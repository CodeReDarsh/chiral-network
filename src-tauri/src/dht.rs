...
async fn handle_dcutr_event(
    ...
    let attempt_num = metrics_guard.dcutr_hole_punch_attempts;

    match result {
        Ok(_connection_id) => {
            metrics_guard.dcutr_hole_punch_successes += 1;
            metrics_guard.last_dcutr_success = Some(SystemTime::now());
            let success_rate = metrics_guard.dcutr_hole_punch_successes as f64
            ...
            let _ = event_tx
                .send(DhtEvent::Info(format!(
                    "✓ Direct connection established with {} via DCUtR hole-punching (attempt #{}, success rate: {:.1}%)",
                    remote_peer_id, attempt_num, success_rate
                )))
                .await;
        }
        Err(error) => {
            ...
            let failure_rate = metrics_guard.dcutr_hole_punch_failures as f64
                / metrics_guard.dcutr_hole_punch_attempts as f64
                * 100.0;
            ...
            let error_type = if error_str.contains("timeout") {
                ...
                "PROTOCOL_ERROR"
            } else if error_str.contains("NAT") {
            ...
            let _ = event_tx
                .send(DhtEvent::Warning(format!(
                    "✗ DCUtR hole-punch to {} failed (attempt #{}, type: {}, error: {}) → using relay fallback",
                    remote_peer_id, attempt_num, error_type, error
                )))
                .await;
        }
    }
